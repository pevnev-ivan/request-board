...
export class CardListComponent implements OnInit {
  ...
  async addCardFast(list: any, email: string) {
    await this.dataService.addListCardFast(list.id, this.boardId!, this.listCards[list.id].length, email)
  }
  ...
  async drop(event: CdkDragDrop<any>) {
    ...
    if (event.container.data[0].list_id === undefined) {
      // this.swapCard(this.listCards[listId])
    }
    ...
    if (event.previousContainer === event.container) {
      ...
      this.listCards[newListId] = this.listCards[newListId].map((card: card, index: number) => {
    ...
    this.listCards[listId] = this.listCards[listId].map((card: any, index: number) => {
  ...
  getConnectedStatuses(listId: number) {
    return this.listCards[listId].filter((list: any) => list.id !== listId).map((list: any) => `index-${list.id}`);
  }
}
